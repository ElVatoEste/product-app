...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <div class="mb-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">{{ __('products.product_information') }}</h2>
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('products.category') }}</label>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $product->description ?? __('products.no_description') }}</p>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('products.created_at') }}</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->created_at->format('d/m/Y H:i:s') }}</p>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('products.updated_at') }}</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->updated_at->format('d/m/Y H:i:s') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <form action="{{ route('products.destroy', $product) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onclick="return confirm('{{ __('products.delete_confirmation') }}')">
                        {{ __('products.delete') }}
                    </button>
                </form>
            </div>
